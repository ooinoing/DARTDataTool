...
import tqdm
from config import DART_API_KEY, DATA_PATH
...
def save_report_no(PATH, market="kospi"):
    """
    ...
    """
    df = pd.read_csv(f"{PATH}/{market}/stock_code.csv")
    df["stock_code"] = df["stock_code"].apply(lambda x: str(x).zfill(6))
    extracted_data = []
    ...
    csv_path = f"{PATH}/{market}/정관/report_no.csv"
...
def download_pdf_files(market="kospi"):
    ...
    df = pd.read_csv(f"{DATA_PATH}/metadata/{MARKET}/report_no.csv")
    ...
    for idx, row in tqdm.tqdm(df.iterrows(), total=df.shape[0]):
        ...
        yy = row["yy"]
        dir_path = f"{DATA_PATH}/report/{MARKET}/20{yy}"
        print(stock_code, corp_name, rcp_no)
    ...
    error_file_path = f"{DATA_PATH}/{MARKET}/error_list.txt"
...
def download_jungwan(market="kospi", output_dir=f"{DATA_PATH}/report/jungwan"):
    """
